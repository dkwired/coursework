...
#define OWNED_EXCLUSION MulSentry exclusion(this,true); exclusion.touch();
#define SHARED_EXCLUSION MulSentry exclusion(this,false); exclusion.touch();
#define waitNreturn tail_wait(); exclusion.suppress_unlock(); return 
...
public:
  ...
  Sentry( Monitor* m ) 
    : mon( *m ), suppress(false),
      old( interrupts.block(mon.mask) )
  { 
    mon.lock(); 
  }
  ~Sentry() {
    if( !suppress ) mon.unlock();
    interrupts.set( old ); 
  }
  void suppress_unlock() { suppress = true;};
};
...
public:
  ...
  void tail_wait( int pr = INT_MAX );    // tail_wait() is defined after CPU
  void signal() { 
    if ( awaited() ) {
      Thread* t = front();
      pop();
...
public:
	...
	void acquire( bool ex );
	void release();
};

void SharableLock::acquire( bool ex = true ) {
	EXCLUSION
	if( ex ) 
	{
		if( state == available ) 
	...
	else 
	{
		...
		{
			okToShare.waitNreturn;
		}
	}
}

void SharableLock::release() {
	EXCLUSION
	--accessCount;
	if( state == owned ) 
		{
		int waiterCount = okToShare.waiting();
		if( waiterCount ) 
		{
			state = shared;
			accessCount += waiterCount;
			okToShare.broadcast();
		}
...
class MulSentry {            // An autoreleaser for monitor's lock.
...
void Condition::tail_wait( int pr ) {
  push( Thread::me(), pr );
  cdbg2 << "releasing CPU just prior to tail wait.\n";
  mon.unlock();
  CPU.release();  
  cdbg2 << "TAIL WAITING\n";
  Thread::me()->suspend();
...
inline void mydelay() { for( int i=0; i< 99999; ++i) {}}
...
public:
	...
	cdbg2 << " BEGIN SHARING\n";
	mydelay();
	typename set<T>::iterator temp = myset.find(x);
	cdbg2 << " END SHARING\n";
	return temp;
}
...
ThreadSafeSet<int> theset; // single instance
...
class Owner : Thread {
	int priority() { return INT_MAX - 1; } // higher-than-last priority
	void action() {
...
// Construct owner threads
Owner own1( " Owner #1" );
Owner own2( " Owner #2" );
Owner own3( " Owner #3" );
// Construct sharer threads
Sharer share1( "Sharer #1" );
Sharer share2( "Sharer #2" );
Sharer share3( "Sharer #3" );
