...
// I, David Klein, certify that all the work (not provided by the TA) is mine.
...
int main(int argc, char *argv[])
{
	...
	char keyfile[] = "rsaprivatekey.pem";
	char shfile[] = "hash-code-signature.bin";

	BIO *bp, *signedhash;
	bp = BIO_new_file(keyfile, "r");
	signedhash = BIO_new_file(shfile, "w");

	RSA * rsa_private = PEM_read_bio_RSAPrivateKey( bp, NULL, NULL, NULL);

	unsigned char enc[128];

	BIO_reset(hash);

//	while( (actualRead = BIO_read(hash, src, 64)) >= 1 )
//	{
		actualRead = RSA_private_encrypt( mdlen, (unsigned char *)mdbuf, enc, rsa_private, RSA_PKCS1_PADDING);
		actualWritten = BIO_write(signedhash, enc, actualRead);
//	}
