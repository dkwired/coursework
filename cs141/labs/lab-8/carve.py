...
# set debug to False before turning in code
debug = True
...
def find_min_energy_seam(image):
    ...
    # Set the remaining rows using the recurrence.
    # TODO: Your Implementation here
    for i in range(1, height):
       for j in range(width):
          for p in range(-1, 2):
             try:
                current = image.energy(i,j) + ME[i-1][j+p]
                if( ME[i][j] is None or ME[i][j] > current ):
                   ME[i][j] = current
                   parents[i][j] = ME[i-1][j+p]
             except IndexError:
                continue
          if( j == width ):
             current1 = image.energy(i,j) + ME[i-1][j-1]
             current2 = image.energy(i,j) + ME[i-1][j]
             if( ME[i][j] is None or ME[i][j] > current1 ):
                ME[i][j] = current1
                parents[i][j] = ME[i-1][j-1]
             if( ME[i][j] is None or ME[i][j] > current2 ):
                ME[i][j] = current2
                parents[i][j] = ME[i-1][j1]
          print ME[1][0]
          print ME[1][571]
          ME[i][j] = image.energy(i,j) + min( ME[i-1][j-1], ME[i-1][j], ME[i][j+1] )
